...
2. 在以下情况下，带声音的自动播放会被允许：
  ...
  2.在桌面设备上，用户的媒体参与度指数阀值已超过，这意味着用户之前播放过有声视频。
  3.用户已将网站添加到移动设备上的主屏幕或在桌面上安装了PWA

3. 顶部帧可以将自动播放权限委派给其iframe，以允许自动播放声音。
  如果父窗体已经有自动播放的能力，将其能力传递给iframe里面的子窗体（前提：同源）

媒体参与度（MEI，Media Engagement Index）
媒体参与度（MEI）衡量个人在网站上使用多媒体的倾向。
它是一个数字，可通过 chrome://media-engagement/ 页面查看。
数值越高，用户对该站点的媒体参与度越高，就越有机会会自动播放。
对于开发者而言：

1. 媒体参与度的计算规则无法通过技术手段更改
2. 媒体参与度的计算规则不同版本的浏览器可能不同

媒体参与度数值越高，浏览器认为该用户对这个网站上的多媒体更感兴趣。浏览器允许其自动播放
